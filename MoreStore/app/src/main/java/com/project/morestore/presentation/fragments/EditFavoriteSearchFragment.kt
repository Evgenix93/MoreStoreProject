...
import com.project.morestore.presentation.dialogs.SuccessSaveSearchDialog
...
class EditFavoriteSearchFragment : FullscreenFragment(), MainMvpView {
    ...
    private fun configureFilterScreen(filter: Filter) {
        ...
        if (filter.chosenForWho.isNotEmpty()) {
            binding.forWomenTextView.text =
                if (filter.chosenForWho[0]) getString(R.string.to_women)
                else if (filter.chosenForWho[1]) getString(R.string.to_men)
                else
                    getString(R.string.to_children)
        }
        ...
        val strings = filter.chosenProductStatus.mapIndexedNotNull { index, b ->
            if (b) {
                when (index) {
                    0 -> getString(R.string.in_stock2)
                    1 -> getString(R.string.booked)
                    2 -> getString(R.string.sold_products)
                    else -> null
                }
            } else
                null
        ...
        if (filter.chosenProductStatus.all { it } || filter.chosenProductStatus.all { !it })
            binding.inStockTextView.text = getString(R.string.instock_booked_sold)
        else
            ...
            if (filter.chosenStyles.all { it.isChecked == true } || filter.chosenStyles.all { it.isChecked == false })
                ...
                filter.chosenStyles.mapIndexedNotNull { index, style ->
                    if (style.isChecked == true)
                        when (index) {
                            0 -> getString(R.string.for_evening)
                            1 -> getString(R.string.for_busyness)
                            2 -> getString(R.string.for_every_day)
                            3 -> getString(R.string.for_sport)
                            else -> null
                        } else
                        null
                }.joinToString(", ")
        ...
        binding.allConditions.text =
            if (filter.chosenConditions.all { it } || filter.chosenConditions.all { !it })
                ...
                filter.chosenConditions.mapIndexedNotNull { index, isChecked ->
                    if (isChecked)
                        when (index) {
                            0 -> getString(R.string.new_with_label)
                            1 -> getString(R.string.new_without_label)
                            2 -> getString(R.string.excellent_condition)
                            3 -> getString(R.string.good_condition)
                            else -> null
                        } else
                        null
                }.joinToString(", ")
    ...
    private fun initToolBar() {
        binding.toolbarFilter.titleTextView.text = getString(R.string.edit_search)
    ...
    private fun saveFilter() {
        if(binding.searchNameEditText.text.isNullOrBlank()){
            Toast.makeText(requireContext(), getString(R.string.write_naming), Toast.LENGTH_SHORT).show()
            return
        }
