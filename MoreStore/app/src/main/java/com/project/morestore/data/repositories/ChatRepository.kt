...
import com.project.morestore.data.apis.MessageApi
...
class ChatRepository @Inject constructor(@ApplicationContext val context: Context, private val chatApi: MessageApi) {
   ...
   suspend fun offerDiscount(info: ChatFunctionInfo): Response<ChatFunctionInfo>?{
    ...
    suspend fun readMessages(dialogId: Id): Response<Id>?{
        return try {
            chatApi.readMessages(dialogId)
        } catch (e: Exception) {
            if (e is IOException) {
                null
            } else {
                Log.d("mylog", e.message.toString())
                try {
                    val response = chatApi.readMessagesGetError(dialogId)
                    if (response.code() == 500) {
                        Response.error(500, "".toResponseBody(null))
                    } else {
                        Response.error(
