...
import com.project.morestore.data.apis.UserApi
...
class UserRepository @Inject constructor(@ApplicationContext val context: Context, private val userApi: UserApi) {
    ...
    suspend fun loadFilter(){
        ...
        withContext(Dispatchers.IO){
            ...
            if (filterJsonString != null){
                ...
                try{
                    val propertiesId = userApi.loadBrandsProperties(Token.userId).body()?.last()?.data
   ...
   suspend fun clearFilter(){
    ...
    suspend fun saveBrandsProperties(brandsId: List<Long>?, propertiesId: List<Long>): Response<Boolean>?{
      return  try {
            userApi.saveBrandsProperties(BrandsPropertiesData(Token.userId, brandsId, propertiesId))
        }catch(e: Throwable){
    ...
    return try {
        ...
        userApi.loadBrandsProperties(Token.userId)
    }catch (e: Throwable){
        Log.e("MyDebug", "error = ${e.message}")
        null
    }
      ...
      return try {
        ...
        if(e is IOException)
            null
              ...
              val error = userApi.getUserError(id)
