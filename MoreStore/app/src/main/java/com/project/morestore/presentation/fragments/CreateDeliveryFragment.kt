...
import com.project.morestore.domain.presenters.CreateDeliveryPresenter
...
class CreateDeliveryFragment: MvpAppCompatFragment(R.layout.fragment_delivery_create), CreateDeliveryMvpView {
    ...
    private fun setDeliveryInfo(order: Order){
        if(order.delivery == 2){
            binding.deliveryInfoTextView.text = getString(R.string.yandex_go)
            binding.dealPlaceTextView.text = getString(R.string.where_take_from)
            ...
            binding.dealPlaceTextView.text = getString(R.string.where_to_deliver_order)
    ...
    override fun success() {
        showMessage(getString(R.string.delivery_order_created))
